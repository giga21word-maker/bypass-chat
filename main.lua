-- // CHRONOS ALPHA 0.0.3 //
-- STATUS: Key-Sensing & Instant Unlock
-- FOCUS: Automated Interaction Chain
...
local function GetKeyInRoom(room)
    -- Scans for keys in drawers or on desks
...
local function AlphaSequence(room)
    ...
    -- STEP 1: Check for Lock & Key
    local doorPart = doorModel:FindFirstChild("Door") or doorModel.PrimaryPart
    local isLocked = doorModel:FindFirstChild("Lock") or doorPart:FindFirstChild("Lock")
    ...
    if isLocked then
        ...
        if keyPrompt then
            ...
            task.wait(0.2) -- Brief delay to let server update inventory
        end
    end

    -- STEP 2: Unlock and Open Door
    local doorPrompt = doorModel:FindFirstChildWhichIsA("ProximityPrompt", true)
    if doorPrompt then
        fireproximityprompt(doorPrompt)
        -- Faster check for opening
        local timeout = 0
        while not doorModel:GetAttribute("Opened") and timeout < 10 do
            task.wait()
            timeout = timeout + 1
        end
    end

    -- STEP 3: Instant Sift TP
    ...
    if nextRoom then
        ...
        if entrance then
            -- Micro-Stutter TP (5 frames) to avoid rubberband
            for i = 1, 5 do
...
task.spawn(function()
    while ALPHA_CONFIG.ACTIVE do
        if ALPHA_CONFIG.AUTO_SKIP then
            -- Find the current room you are in/near
...
local function BuildUI()
    ...
    Main.BackgroundColor3 = Color3.fromRGB(25, 20, 20)
    Instance.new("UICorner", Main)
    
    local B = Instance.new("TextButton", Main)
    B.Size = UDim2.new(1, -20, 1, -20)
    B.Position = UDim2.new(0, 10, 0, 10)
    B.Text = "AUTO-KEY-SKIP: OFF"
    B.BackgroundColor3 = Color3.fromRGB(45, 35, 35)
    ...
    B.MouseButton1Down:Connect(function()
        ...
        B.Text = ALPHA_CONFIG.AUTO_SKIP and "KEY-SKIP: ON" or "KEY-SKIP: OFF"
        B.TextColor3 = ALPHA_CONFIG.AUTO_SKIP and Color3.new(1, 0.4, 0.4) or Color3.new(1, 1, 1)
    end)
end
